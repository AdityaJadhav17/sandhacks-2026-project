...
from agents.trip_planner.analyst.agent_executor import AnalystAgentExecutor
...
factory = AgntcyFactory("lungo.trip_planner.analyst", enable_tracing=True)

logger = logging.getLogger("lungo.trip_planner.analyst.server")


async def liveness_probe(request) -> JSONResponse:
    """Health check endpoint."""
    return JSONResponse({"status": "alive", "agent": "analyst"})
...
async def run_http_server(server):
    ...
    try:
        config = Config(app=app_, host="0.0.0.0", port=9101, loop="asyncio")
