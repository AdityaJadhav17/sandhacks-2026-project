...
logger = logging.getLogger("lungo.trip_planner.planner.agent_executor")
...
class PlannerAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        """
        Execute the Planner agent's itinerary creation logic.

        Args:
            context: The request context containing the filtered options.
            event_queue: The queue to publish events to.
        """
        logger.debug("Planner received message request: %s", context.message)
        ...
        try:
            ...
            logger.debug("Planner agent output generated")
