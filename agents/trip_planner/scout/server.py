...
from agents.trip_planner.scout.agent_executor import ScoutAgentExecutor
...
factory = AgntcyFactory("lungo.trip_planner.scout", enable_tracing=True)

logger = logging.getLogger("lungo.trip_planner.scout.server")


async def liveness_probe(request) -> JSONResponse:
    """Health check endpoint."""
    return JSONResponse({"status": "alive", "agent": "scout"})
...
def create_app() -> FastAPI:
    request_handler = DefaultRequestHandler(
        agent_executor=ScoutAgentExecutor(),
...
async def run_http_server(server):
    ...
    try:
        config = Config(app=app_, host="0.0.0.0", port=9100, loop="asyncio")
